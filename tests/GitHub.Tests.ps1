...
Describe 'GitHub' {
    ...
    Context 'IssueParser' {
        It 'ConvertFrom-IssueBody - Should return a PSCustomObject' {
            $issueTestFilePath = Join-Path -Path $PSScriptRoot -ChildPath 'IssueBody.md'
            $data = Get-Content -Path $issueTestFilePath -Raw | ConvertFrom-IssueBody
            Write-Verbose ($data | Format-Table | Out-String) -Verbose
            $data | Should -BeOfType 'PSCustomObject'
        }

        It 'ConvertFrom-IssueBody -AsHashtable - Should return a hashtable' {
            $issueTestFilePath = Join-Path -Path $PSScriptRoot -ChildPath 'IssueBody.md'
            $data = Get-Content -Path $issueTestFilePath -Raw | ConvertFrom-IssueBody -AsHashtable
