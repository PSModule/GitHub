...
Describe 'As a GitHub App - Enterprise (APP_ENT)' {
    ...
    Context 'Secrets' {
        Context 'Get-GitHubPublicKey' {
            It 'retrieves a public key for the authenticated user' {
                $result = Get-GitHubPublicKey
                $result | Should -Not -BeNullOrEmpty
                $result.key | Should -Not -BeNullOrEmpty
                $result.key_id | Should -Not -BeNullOrEmpty
            }

            It 'retrieves a public key for the organization' {
                $result = Get-GitHubPublicKey -Organization 'psmodule-test-org3'
                $result | Should -Not -BeNullOrEmpty
                $result.key | Should -Not -BeNullOrEmpty
                $result.key_id | Should -Not -BeNullOrEmpty
            }

            It 'retrieves a public key for the repository' {
                $result = Get-GitHubPublicKey -Owner $TestOwner -Repository $TestRepository -Type 'codespaces'
                $result | Should -Not -BeNullOrEmpty
                $result.key | Should -Not -BeNullOrEmpty
                $result.key_id | Should -Not -BeNullOrEmpty
            }
        ...
        Context 'Repository Secret Lifecycle' {

            It 'sets a repository secret' {
                # Convert the plain text secret to a SecureString
                $secureValue = ConvertTo-SecureString $TestSecretValue -AsPlainText -Force

                # Set the repository secret (using -Confirm:$false to bypass ShouldProcess)
                $null = Set-GitHubSecret -Owner $TestOwner -Repository $TestRepository -Name $TestSecretName -Value $secureValue -Confirm:$false

                # Allow a few seconds for the secret to propagate
                Start-Sleep -Seconds 2

                # Verify that the secret exists
                $exists = Test-GitHubSecret -Owner $TestOwner -Repository $TestRepository -Name $TestSecretName
                $exists | Should -BeTrue
            }

            It 'gets the repository secret' {
                $secret = Get-GitHubSecret -Owner $TestOwner -Repository $TestRepository -Name $TestSecretName
                $secret.name | Should -Be $TestSecretName
            }

            It 'removes the repository secret' {
                $null = Remove-GitHubSecret -Owner $TestOwner -Repository $TestRepository -Name $TestSecretName -Confirm:$false
                Start-Sleep -Seconds 2
        ...
        Context 'Organization Secret Lifecycle' {

            It 'sets an organization secret' {
                $secureValue = ConvertTo-SecureString $TestSecretValue -AsPlainText -Force
                $null = Set-GitHubSecret -Organization 'psmodule-test-org3' -Name $TestSecretName -Value $secureValue -Confirm:$false
                Start-Sleep -Seconds 2

                $exists = Test-GitHubSecret -Organization 'psmodule-test-org3' -Name $TestSecretName
                $exists | Should -BeTrue
            }

            It 'gets the organization secret' {
                $secret = Get-GitHubSecret -Organization 'psmodule-test-org3' -Name $TestSecretName
                $secret.name | Should -Be $TestSecretName
            }

            It 'removes the organization secret' {
                $null = Remove-GitHubSecret -Organization 'psmodule-test-org3' -Name $TestSecretName -Confirm:$false
                Start-Sleep -Seconds 2
        ...
        Context 'Authenticated User Secret Lifecycle' {
            It 'sets an authenticated user secret' {
                $secureValue = ConvertTo-SecureString $TestSecretValue -AsPlainText -Force
                $null = Set-GitHubSecret -Name $TestSecretName -Value $secureValue -Confirm:$false
                Start-Sleep -Seconds 2

                $exists = Test-GitHubSecret -Name $TestSecretName
                $exists | Should -BeTrue
            }

            It 'gets the authenticated user secret' {
                $secret = Get-GitHubSecret -Name $TestSecretName
                $secret.name | Should -Be $TestSecretName
            }

            It 'removes the authenticated user secret' {
                $null = Remove-GitHubSecret -Name $TestSecretName -Confirm:$false
                Start-Sleep -Seconds 2
